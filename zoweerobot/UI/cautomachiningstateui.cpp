...
//                                "}");
    ...
    ui->axis5_pulse_multi->setNum(0);
     ...
     if((0x40==(m_pScheduler->m_pSystemParameter->sys_ctrl.statebit&0x60)) // Ä£Ê½Ö¸Áî Îª×Ô¶¯·½Ê½
     &&(m_pScheduler->m_pSystemParameter->sys_ctrl.decode_id<=cnt)
     && (!(m_pScheduler->m_pSystemParameter->sys_ctrl.statebit&0x400)))  // ÊÇ·ñÍ£Ö¹×´Ì¬
     {
...
void CAutoMachiningStateUi::on_addtechpoint_clicked()
{
    ...
    if(m_pScheduler->m_pSystemParameter->coor_joint_pos[1]>0.000001){  // ÊÖÏµ
        hand = 1;
